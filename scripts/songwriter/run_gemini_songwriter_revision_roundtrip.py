...
from src.akira_engine.songwriter_v2_exchange import import_prediction_bundle
from src.akira_engine.songwriter_v2_revision import (
    build_revision_request_bundle,
    load_json,
    render_revision_comparison_report,
    render_revision_request_report,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a critic-guided Gemini revision pass for low-scoring Songwriter V2 outputs.",
    )
    parser.add_argument(
        "--scoring-manifest",
        required=True,
    ...
    parser.add_argument(
        "--work-dir",
        type=Path,
        default=Path("outputs") / "gemini_songwriter_revision_roundtrip",
        help="Directory where request, prediction, and import artifacts will be written.",
    )
    parser.add_argument(
        "--score-dir",
        type=Path,
        default=Path("reports") / "quality" / "gemini_songwriter_revision_roundtrip",
    ...
    parser.add_argument(
        ...
        type=float,
        default=88.0,
        help="Revise tracks whose score is below this threshold.",
    )
    parser.add_argument(
        "--max-revisions",
        type=int,
        help="Optional cap on the number of revisions to request.",
    )
    parser.add_argument(
        "--model",
...
def main() -> None:
    ...
    request_bundle = build_revision_request_bundle(
        scoring_manifest,
    ...
    request_report_path.write_text(render_revision_request_report(request_bundle), encoding="utf-8")

    if request_bundle["request_count"] == 0:
        print("No revision requests were generated.")
        return
    ...
    revised_scoring = score_external_predictions(run_root, work_dir / "imported_predictions", score_dir)
    report_path = score_dir / "revision_roundtrip_report.md"
    report_path.write_text(render_scoring_report(revised_scoring), encoding="utf-8")

    comparison_report_path = score_dir / "revision_comparison_report.md"
    comparison_report_path.write_text(
        render_revision_comparison_report(original_scoring, revised_scoring),
        encoding="utf-8",
    )
    ...
    print(f"Revised scoring report: {report_path}")
