...
from akira_engine.round2_seed_external import validate_round2_seed_payload


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate round2 seed payloads before merge.")
...
def render_report(payload: dict[str, object]) -> str:
    lines = [
        f"# Round2 Seed Validation: {payload['artist_id']}",
        "",
...
def main() -> None:
    ...
    for path in sorted(input_dir.glob("*.json")):
        ...
        issues = validate_round2_seed_payload(payload)
        track_id = str(payload.get("track_id", "")).strip()
        results.append({"filename": path.name, "track_id": track_id, "valid": not issues, "issues": issues})
    ...
    write_utf8_json(out_dir / f"{args.artist_id}_round2_seed_external_validation.json", output)
    write_utf8_text(
        out_dir / f"{args.artist_id}_round2_seed_external_validation.md",
