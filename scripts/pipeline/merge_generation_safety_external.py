...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Merge generation_safety invalid remediation files into reference_tracks.")
...
def render_report(payload: dict[str, object]) -> str:
    lines = [
        f"# Generation Safety Conditioning Merge: {payload['artist_id']}",
        "",
...
def main() -> None:
    ...
    backup_dir = None
    if args.backup:
        backup_dir = project_root / "reports" / "quality" / "conditioning_merge_backups" / "generation_safety" / args.artist_id
    ...
    manifest_path = write_utf8_json(output_dir / f"{args.artist_id}_generation_safety_conditioning_merge.json", payload)
    report_path = write_utf8_text(
        output_dir / f"{args.artist_id}_generation_safety_conditioning_merge.md",
