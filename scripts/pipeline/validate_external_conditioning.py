...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate external conditioning JSON files before merge.")
...
def render_report(payload: dict[str, object]) -> str:
    lines = [
        f"# External Conditioning Validation: {payload['artist_id']}",
        "",
...
def main() -> None:
    ...
    manifest_path = write_utf8_json(out_dir / f"{args.artist_id}_external_validation.json", output)
    report_path = write_utf8_text(out_dir / f"{args.artist_id}_external_validation.md", render_report(output), trailing_newline=False)
    print(manifest_path)
    print(report_path)
