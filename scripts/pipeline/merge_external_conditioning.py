...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Merge externally enriched conditioning JSON files into reference_tracks.")
    parser.add_argument("--artist-id", required=True, help="Artist or producer id, e.g. deco27")
    parser.add_argument("--input-dir", required=True, help="Directory containing external JSON files")
    parser.add_argument("--project-root", default=".", help="Project root path")
    parser.add_argument("--backup", action="store_true", help="Write backups before overwriting target files")
    return parser.parse_args()


def render_report(payload: dict[str, object]) -> str:
    lines = [
        f"# Conditioning Merge: {payload['artist_id']}",
        "",
...
def main() -> None:
    ...
    backup_dir = None
    if args.backup:
        backup_dir = project_root / "reports" / "quality" / "conditioning_merge_backups" / args.artist_id
    ...
    manifest_path = write_utf8_json(output_dir / f"{args.artist_id}_conditioning_merge.json", payload)
    report_path = write_utf8_text(
        output_dir / f"{args.artist_id}_conditioning_merge.md",
