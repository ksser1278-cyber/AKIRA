...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Merge producer expansion external conditioning JSON files.")
...
def render_report(payload: dict[str, object]) -> str:
    lines = [
        f"# Producer Expansion Conditioning Merge: {payload['artist_id']}",
        "",
...
def main() -> None:
    ...
    backup_dir = None
    if args.backup:
        backup_dir = project_root / "reports" / "quality" / "conditioning_merge_backups" / args.artist_id / "producer_expansion"
    ...
    manifest_path = write_utf8_json(output_dir / f"{args.artist_id}_producer_expansion_conditioning_merge.json", payload)
    report_path = write_utf8_text(
        output_dir / f"{args.artist_id}_producer_expansion_conditioning_merge.md",
