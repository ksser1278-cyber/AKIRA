...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate round2 external conditioning JSON files before merge.")
...
def render_report(payload: dict[str, object]) -> str:
    lines = [
        f"# Round2 External Validation: {payload['artist_id']}",
        "",
...
def main() -> None:
    ...
    manifest_path = write_utf8_json(out_dir / f"{args.artist_id}_round2_external_validation.json", output)
    report_path = write_utf8_text(
        out_dir / f"{args.artist_id}_round2_external_validation.md",
