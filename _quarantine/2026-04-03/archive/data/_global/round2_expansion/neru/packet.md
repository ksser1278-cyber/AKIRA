...
- `neru_tokyo_teddy_bear` / status `validated` / priority `high` / mode `direct_emotional_pop`
- `neru_lost_ones_weeping` / status `scaffolded` / priority `high` / mode `direct_emotional_pop`
- `neru_law_evading_rock` / status `scaffolded` / priority `high` / mode `ironic_meta`
- `neru_snobbism` / status `scaffolded` / priority `medium` / mode `ironic_meta`
- `neru_abstract_nonsense` / status `scaffolded` / priority `medium` / mode `ironic_meta`
...
- `C:\JPop_Songwriter\AKIRA ENGINE\data\neru\reference_tracks\round2_seed_scaffolds`

## Validated Tracks

- `neru_tokyo_teddy_bear` / score `87` / keep as benchmarked round2 winner
